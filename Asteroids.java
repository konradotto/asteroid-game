...
public class Asteroids extends JPanel implements Runnable, KeyListener {
  ...
  static final int HYPER_COUNT  = 3 * AsteroidsController.getFps();  // calculated using number of
  static final int MISSLE_COUNT = 4 * AsteroidsController.getFps();  // seconds x frames per second.
  static final int STORM_PAUSE  = 2 * AsteroidsController.getFps();
  ...
  {
	 Asteroids myApp = new Asteroids();
     myApp.setBounds(0,0,1200,800);    
     myApp.init();
     ...
     mainFrame.addComponentListener(new ComponentAdapter() {
         public void componentResized(ComponentEvent e) {
       	  myApp.setBounds(0, 0, e.getComponent().getWidth(), e.getComponent().getHeight());
       	  myApp.setSize(e.getComponent().getWidth(), e.getComponent().getHeight());
       	  AsteroidsSprite.width = e.getComponent().getWidth();
          AsteroidsSprite.height = e.getComponent().getHeight();
          myApp.repaint();
         }
     });
